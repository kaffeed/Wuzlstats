...
public class PostScoreViewModel
{
    ...
    private async Task SavePlayerScore(League league, Db db)
    {
        ...
        // Players need to be saved first to get IDs
        var bluePlayer = await GetOrCreatePlayer(BluePlayer, league, db);
        var redPlayer = await GetOrCreatePlayer(RedPlayer, league, db);

        // Now create game and positions, then save all at once
        var game = CreateGame(RedPlayerScore, BluePlayerScore, league, db);
        CreatePosition(game, redPlayer, PlayerPositionTypes.Red, db);
        CreatePosition(game, bluePlayer, PlayerPositionTypes.Blue, db);
    ...
    private async Task SaveTeamScore(League league, Db db)
    {
        ...
        // Now create game and positions, then save all at once
        var game = CreateGame(RedTeamScore, BlueTeamScore, league, db);
        CreatePosition(game, blueOffense, PlayerPositionTypes.BlueOffense, db);
        CreatePosition(game, blueDefense, PlayerPositionTypes.BlueDefense, db);
        CreatePosition(game, redOffense, PlayerPositionTypes.RedOffense, db);
        CreatePosition(game, redDefense, PlayerPositionTypes.RedDefense, db);
    ...
    private Game CreateGame(int redScore, int blueScore, League league, Db db)
    {
        var game = new Game
        {
            BlueScore = blueScore,
    ...
    private void CreatePosition(Game game, Models.Player player, PlayerPositionTypes position, Db db)
    {
